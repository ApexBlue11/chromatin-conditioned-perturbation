...
| 15 | **TPU v5e multi-process init fails on a single Kaggle VM.** After #13 was fixed, `preflight` passed, spawn started all 8 processes, and every one died in `xm.get_xla_supported_devices()` with `TPU initialization failed: Invalid --..._slice_builder_worker_addresses specified. Expected 8 worker addresses, got 1` — the runtime wants a multi-host slice, not 8 processes on one host (same class as pytorch-lightning #20244) | all 8 replicas abort (signal 6) after another full queue wait | **Not solved — we stopped paying for it.** Two queue cycles produced zero training, and the TPU was already measured *slower per core than a T4* [8]. **Use `train_v6_gpu.py` on T4 x2.** If TPU is ever revisited: single-process SPMD, not `xmp.spawn` per device |
