...
def rec(diff=-0.05, p=0.001, ps=0.001, S=0.30, null2=0.40, un_diff=-0.05, un_p=0.001, rho=0.5, untrained=False,
        epoch=11, q='q1', void=False, out_diff=0.0, out_p=0.5, data_diff=0.0, data_p=0.5):
    sc = lambda d, pp: {'S': S, 'null1_mean': S - d, 'null1_sd': 0.01, 'diff': d, 'p': pp, 'null2_mean': null2, 'n': 496}
    return {'void': void, 'row_sha1': ROWS, 'quintile_sha1': q, 'n_compounds': 496, 'max_da': 1.0,
            ...
            'primary': {'score': sc(diff, p), 'null1s': {'S': S, 'null1s_mean': S - diff, 'null1s_sd': 0.01,
                                                          'diff_s': diff, 'p_s': ps}},
            'output_projection': {'score': sc(out_diff, out_p)}, 'data_projection': {'score': sc(data_diff, data_p)},
...
U = [rec(diff=d, un_diff=d, p=0.4, ps=0.4, untrained=True) for d in (-0.004, -0.006, -0.002, -0.008, -0.005)]
T = [rec() for _ in range(3)]


def r(T_, U_=U):
    return read(copy.deepcopy(T_), copy.deepcopy(U_))
...
def test_signal_qualifiers_drop_when_projections_aligned_on_any_seed():
    T2 = [rec(), rec(data_diff=-0.03, data_p=0.01), rec(out_diff=-0.03, out_p=0.01)]
    o = r(T2)
    assert o['reading'] == 'SIGNAL'
...
def test_null_when_not_below_untrained_minimum():
    U2 = copy.deepcopy(U)
    U2[3] = rec(diff=-0.06, un_diff=-0.06, p=0.4, ps=0.4, untrained=True)      # one init reaches below the trained
    assert r(T, U2)['reading'] == 'NULL'


def test_null_when_not_below_mean_minus_2sd():
    U2 = [rec(diff=d, un_diff=-0.004, p=0.4, ps=0.4, untrained=True) for d in (-0.049, 0.02, 0.03, -0.01, 0.0)]
