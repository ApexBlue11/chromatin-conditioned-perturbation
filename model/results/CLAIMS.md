...
| # | Claim | Status |
|---|---|---|
| 7.1 | Conditioning transcriptional-RESPONSE-PROFILE prediction on cell-line chromatin state is novel | **NOVELTY still B** (4 targeted searches; response-profile models use expression/mutation/structure, not chromatin). 🔴 **BUT THE BENEFIT IS NOW MEASURED AND ABSENT.** §44: with the cell's own baseline expression already an input, chromatin adds **+0.0002 / +0.0003 / +0.0029** delta Pearson to a ridge on warm / cold-drug / **cold-cell** splits of XPert's benchmark — an exact rank-preserving encoding, penalty swept, 94.3 % of cold-cell test rows covered. §43.2 agrees from the deep model. For scale, a ridge trails XPert by 0.087. **Do not claim a chromatin BENEFIT; the novelty of the idea is not evidence that it works.** The deep-model ablation (`--ablate_epi`, 3 seeds, cold-cell) is written and queued, blocked on GPU quota | **novelty B / benefit ✗ MEASURED ABSENT** |
