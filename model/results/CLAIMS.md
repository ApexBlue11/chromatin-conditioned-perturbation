...
|---|---|---|---|
| 1.1 | An interpretable (non-VAE, attention-attributable) model predicts LINCS L1000 differential response well above trivial baselines | v5 cold-cell MSE **1.481** vs Mean/Meancell **1.747**, Meandrug **1.733** | **A** |
...
| 1.5 | **Our model generalizes BETTER to new compounds than to new cell lines** (0.471 vs 0.440) — cell-specificity, not chemistry, is the hard part | v5 three-way split, one run | **A** |
...
| # | Claim | Evidence | Strength |
|---|---|---|---|
| 4.1 | Atom→gene attention localizes to known drug targets, **strengthening monotonically with reference confidence**: 0.9× (all 19,174 noisy edges) → **2.1×** (447 ChEMBL curated) → **2.6×** (156 both-source gold) recall@5 | `dti_eval.json` | **A** |
...
| 6.6f | The DEFENSIBLE version: absolute-convention PCC is **highly sensitive to the basal:delta variance ratio** (0.485→0.999 across ratios 0→1000 on fixed predictions), therefore absolute-convention numbers are **uninterpretable across protocols unless that ratio (and the basal source) is reported** | 6.6c | **A** |
...
| # | Claim | Status |
|---|---|---|
| 7.1 | Conditioning a drug-perturbation predictor on cell-line CHROMATIN state appears novel | **B** (upgraded 2026-07-27). Targeted searches find only ADJACENT work, none the same task: PERD predicts chromatin change FROM perturbed expression (reverse direction); NAR 2022 infers transcriptional programs from *somatic alterations* (different driver); eLife 78012 does integrated transcriptome+chromatin *analysis*, not a conditioned predictor; Perturb-seq / Spear-ATAC are *measurement* methods. **Still not a systematic review** — a proper database search is owed before asserting priority |
