...
| M.10 | 🔴 **MEASURED SEED VARIANCE IS LARGER THAN ALMOST EVERY EFFECT THIS PROJECT HAS CLAIMED** (2026-08-15, first time two seeds of one identical config were ever run). Two v7 seeds, same data, same config, same protocol: unseen-cell **0.4322 / 0.4374** (range 0.0052), unseen-compound **0.4448 / 0.4624** (range **0.0176**), unseen-both **0.4237 / 0.4532** (range **0.0295**). For comparison the v6−v5 differences we were interpreting were **+0.0066 / −0.0024 / +0.0153** — *all inside the seed range on the same split*. ⇒ **v6 vs v5 was never resolvable**, and no single-run comparison below ~0.03 in this project ever was. R² is worse: seed 0 vs 1 on unseen-both is +0.1705 vs +0.2411. **Report mean ± range over ≥3 seeds, or do not report a difference at all.** This supersedes the "single seed" caveat in M.3 with a measured number | `eval_v7.py`, seeds 0/1, protocol-matched | **A** |
