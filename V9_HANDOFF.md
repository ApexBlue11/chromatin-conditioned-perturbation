...
| **Drug features are the model** 🔴 **OVERREACH — see RESULTS §50.** This is an INFERENCE-time ablate-to-mean, which shows the trained model USES them. Bai et al. (bioRxiv 2026-05) RETRAINED seven L1000 models from scratch with drug inputs zeroed and saw ≤0.012 change, with a drug-free MLP scoring 0.637 against XPert's 0.633 — so the TASK may not require them. Both can hold. Rewrite before claiming. | Ablate-to-mean: drug global **+0.25…+0.30**; baseline expression +0.026…+0.045; atom tokens +0.002…+0.024; lineage +0.002…+0.022. |
...
11. **Size the experiment against a MEASURED noise floor before buying it.** The chromatin effect is 0.57 sigma of the run-to-run difference using this project’s own unseen-cell seed sd of 0.0052 [§21]; three seeds per arm would have cost 62 GPU-hours to re-measure a known spread, and powering the effect would cost ~250 [RESULTS §52]. Also: §30’s "0.005–0.045" are RANGES, not sds. **AMENDED [§53.4]: check WHAT KIND of statistic the measurement is — ranges, sds and sems all render as "the spread" in prose, and in this exchange two parties in a row misread a measured number whose definition neither had checked. Also: a "~n runs to power it" figure is the 51 % point unless you used (z_a + z_b); the 80 % number is ~2x larger.**
12. **Before buying compute, enumerate what is already on disk — including gitignored artefact directories.** §52 was about to spend 21 GPU-hours re-generating predictions that had been in `external/v9_mdmt_preds/` since August, and the decision-grade version of that experiment would have cost 499 h [RESULTS §53.1, §54]. Twice now this project has paid for something already in hand (§46.1 supplementary tables, §54.1 saved predictions).
13. **Fix the estimand and its estimator BEFORE computing the number.** On 8 cell-line clusters, five defensible estimators of the same effect spanned two signs and a 4x magnitude range (+0.0042 size-weighted / −0.0141 unweighted / −0.0060 trimmed / −0.0038 drop-one / +0.0004 treated-only) [RESULTS §55.3]. Choosing after seeing them is how §45 got a positive and §54 got a negative from the same data. Also: **units with no treatment do not belong in a treatment estimate** — the 3 cells with no chromatin supplied 102 % of §54’s negative.
