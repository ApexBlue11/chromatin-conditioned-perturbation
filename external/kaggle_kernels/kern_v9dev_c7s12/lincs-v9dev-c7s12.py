...
# RESULTS 85.7 candidate C7: --chromatin_edges, seeds 1..2 (rule 6 ADVANCE, RESULTS 85.8), the P2 command otherwise.
sys.argv = ['xpert_arm.py', '--bundle', 'xpert_mdmt_splits.npz', '--split', 'split_cold_cell_1',
            '--dev_cells', '6', '--dev_seed', '0', '--dp_seed_mode', 'distinct',
            '--seeds', '2', '--seed_start', '1', '--epochs', '12', '--d_model', '256',
            '--save_pred', '/kaggle/working/v9dev_c7.npz', '--save_ckpt', '/kaggle/working/v9dev_c7.pt'] + ['--chromatin_edges']
xpert_arm.main()

# GUARD 4 (RESULTS 85.4): the carve must be the recorded one, row for row.
import json as _json
outs = glob.glob('/kaggle/working/**/v9_xpert_arm_*_dev6s0_chromedges.json', recursive=True) +        glob.glob(os.path.join(os.path.dirname(SRC), '**', 'v9_xpert_arm_*_dev6s0_chromedges.json'), recursive=True)
if not outs:
