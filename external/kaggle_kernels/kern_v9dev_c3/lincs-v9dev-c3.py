...
# RESULTS 85.7 candidate C3: --listnet_w 1.116903, seeds 0..0, the P2 command otherwise.
sys.argv = ['xpert_arm.py', '--bundle', 'xpert_mdmt_splits.npz', '--split', 'split_cold_cell_1',
            ...
            '--save_pred', '/kaggle/working/v9dev_c3.npz', '--save_ckpt', '/kaggle/working/v9dev_c3.pt'] + ['--listnet_w', '1.116903']
xpert_arm.main()

# GUARD 4 (RESULTS 85.4): the carve must be the recorded one, row for row.
import json as _json
outs = glob.glob('/kaggle/working/**/v9_xpert_arm_*_dev6s0_listnet.json', recursive=True) +        glob.glob(os.path.join(os.path.dirname(SRC), '**', 'v9_xpert_arm_*_dev6s0_listnet.json'), recursive=True)
if not outs:
