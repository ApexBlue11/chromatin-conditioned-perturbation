...
if not all(k in _src for k in ('def carve_dev', 'dev_cells', 'dev_row_index_sha1', 'original_test_row_indices',
                              'def seed_devices', 'cuda_rng_states_equal_by_epoch')):
    raise SystemExit('FATAL: mounted xpert_arm.py lacks the dev-cell mode. Refusing.')

# RESULTS 85.5 (review 021 C1), verified FIRST: under the pre-85.5 seeding do the two GPUs' CUDA generators stay in
# lockstep? A 1-epoch, 960-row run in `lockstep` mode; no predictions or checkpoint are kept.
import json as _json
sys.argv = ['xpert_arm.py', '--bundle', 'xpert_mdmt_splits.npz', '--split', 'split_cold_cell_1',
            '--dev_cells', '6', '--dev_seed', '0', '--dp_seed_mode', 'lockstep',
            '--seeds', '1', '--seed_start', '0', '--epochs', '1', '--d_model', '256', '--limit_train', '960']
xpert_arm.main()
_chk = _json.load(open('/kaggle/working/v9_xpert_arm_split_cold_cell_1_seed0_dev6s0.json'))
LOCKSTEP = _chk['runs'][0]
print('LOCKSTEP CHECK', _json.dumps({k: LOCKSTEP[k] for k in ('n_gpu', 'dp_seed_mode', 'device_seeds',
                                                             'cuda_rng_states_equal_by_epoch')}), flush=True)
os.rename('/kaggle/working/v9_xpert_arm_split_cold_cell_1_seed0_dev6s0.json', '/kaggle/working/lockstep_check.json')

# RESULTS 85 P2: the BASELINE on the 6 dev cells, seeds 0-2 (fixed), 12 epochs, V9Config defaults.
# The 8 test cells are removed from every array in dev mode; nothing here can score them.
sys.argv = ['xpert_arm.py', '--bundle', 'xpert_mdmt_splits.npz', '--split', 'split_cold_cell_1',
            '--dev_cells', '6', '--dev_seed', '0',
            '--seeds', '3', '--seed_start', '0', '--epochs', '12', '--d_model', '256',
            '--save_pred', '/kaggle/working/v9dev_base.npz',
            '--save_ckpt', '/kaggle/working/v9dev_base.pt']
xpert_arm.main()

# GUARD 4 (RESULTS 85.4): the carve must be the recorded one, row for row.
import json as _json
outs = glob.glob('/kaggle/working/**/v9_xpert_arm_*_dev6s0.json', recursive=True) +        glob.glob(os.path.join(os.path.dirname(SRC), '**', 'v9_xpert_arm_*_dev6s0.json'), recursive=True)
if not outs:
