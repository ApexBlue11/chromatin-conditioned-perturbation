...
# RESULTS 85.7 candidate C2: --l_control 0, seeds 0..0, the P2 command otherwise.
sys.argv = ['xpert_arm.py', '--bundle', 'xpert_mdmt_splits.npz', '--split', 'split_cold_cell_1',
            ...
            '--save_pred', '/kaggle/working/v9dev_c2.npz', '--save_ckpt', '/kaggle/working/v9dev_c2.pt'] + ['--l_control', '0']
xpert_arm.main()

# GUARD 4 (RESULTS 85.4): the carve must be the recorded one, row for row.
import json as _json
outs = glob.glob('/kaggle/working/**/v9_xpert_arm_*_dev6s0_lctl0.json', recursive=True) +        glob.glob(os.path.join(os.path.dirname(SRC), '**', 'v9_xpert_arm_*_dev6s0_lctl0.json'), recursive=True)
if not outs:
